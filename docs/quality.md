...
### What we don’t over-abstract

- Keep interfaces minimal (only the methods you use).
- Avoid “wrapper classes” without a clear testing/portability reason.
...
- When we know the failure class, we throw a typed `DiskcareError` with:
  - `code` (stable identifier)
  - `context` (safe-to-print metadata)
  - `cause` (original error)
- `--verbose` prints stack trace + cause chain.
...
- **DI**: IO bağımlılıklarını (fs, shell, output) inject ederek testleri hızlı ve stabil tut.
- **Hatalar**: CLI’de tek bir error handler; mümkünse `DiskcareError(code, context, cause)`.
- **Test izolasyonu**: gerçek disk yerine in-memory fake fs; platform bağımlı path hatalarını `path.join()` ile önle.
